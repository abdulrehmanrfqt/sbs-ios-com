...
export const switchProfile=()=>(dispatch)=>{
    dispatch({
        type:SWITCH_PROFILE,
        payload:{}
    })
...
export const saveToken=(t)=>async(dispatch)=>{
    console.log(t)
    const token=await AsyncStorage.getItem('token')
    const res=await sbs.post(`/api/v1/customer/token`,{token:t},{
        headers:{
            Authorization:token
        }
    });
...
export const forgetPassword=(email)=>async(dispatch)=>{
    const token=await AsyncStorage.getItem('token')
    const res=await sbs.post(`/api/v1/customer/forget-password`,{email},{
        headers:{
            Authorization:token
        }
    });
